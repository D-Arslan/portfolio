  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        '    <div class="cap"></div>' +
        '    <div class="sub">ou <u>parcourir les fichiers</u></div></div>' +
        ...
        '</div>' +
        '<div class="ctl"><button data-act="replace" title="Remplacer l’image">Remplacer</button>' +
        '  <button data-act="clear" title="Retirer l’image">Retirer</button></div>' +
    ...
    async _ingest(file) {
      ...
      if (!file || ACCEPT.indexOf(file.type) < 0) {
        this._setError('Déposez une image PNG, JPEG, WebP ou AVIF.');
        return;
      }
      ...
      try {
        ...
        if (gen !== this._gen) return;
        this._setError('Impossible de lire cette image.');
    ...
    _render() {
      ...
      this._cap.textContent = this.getAttribute('placeholder') || 'Déposez une image';
